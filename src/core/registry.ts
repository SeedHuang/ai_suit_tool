 ...
 *
 * **两条路径都返回副本** —— 直接回表内对象会让调用方就地一改(按 UI 校正数字)
 * 就永久污染全局 MODELS 表,后续所有请求都拿到被改过的值。
 */
export function getModelMeta(provider: string, model: string): ModelMeta {
  const hit = MODELS[model];
  return hit ? { ...hit } : { ...FALLBACK, provider, model };
}

/**
 * 列模型 —— 设置页的模型下拉用;不传 provider 则全列。
 *
 * 返回的同样是**副本**(与 getModelMeta 一致)—— 直接把表内对象交出去,调用方就地一改
 * (按 UI 校正数字)就会永久污染全局 MODELS 表,后续所有请求都拿到被改过的值。
 */
export function listModels(provider?: string): ModelMeta[] {
  const all = Object.values(MODELS).map((m) => ({ ...m }));
