...
describe('listRemoteModels', () => {
  ...
  // ★ A4：端点只连不答（TCP 黑洞 / 网关不响应）时不能永挂 —— 到点抛一句能读的话。
  //   底层假 fetch 刻意**不理会 abort**：超时不能指望它配合。
  it('端点挂起 → 超时抛一句能读的话(不永挂)', async () => {
    const hung = (() => new Promise<Response>(() => {})) as unknown as typeof fetch;
    await expect(
      listRemoteModels({ provider: 'deepseek', apiKey: 'k', fetchImpl: hung, timeoutMs: 20 }),
    ).rejects.toThrow(/拉模型列表超时\(20ms\)/);
  });

  it('abortSignal 透传给 fetch(调用方一 abort,请求就中断)', async () => {
    const controller = new AbortController();
    let seen: AbortSignal | null | undefined;
    // 真实 fetch 就是「signal 一 abort 立刻 reject」—— 这里照着模拟，验证信号真的透传到了
    ...
    const pending = listRemoteModels({
      provider: 'deepseek', apiKey: 'k', fetchImpl: impl, abortSignal: controller.signal,
    });
    await new Promise((r) => setTimeout(r, 0)); // 等 fetch 真的被调起来
