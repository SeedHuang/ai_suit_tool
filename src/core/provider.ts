...
/** 网络请求的默认上限(ms) —— 端点黑洞时不该有任何一个 await 永久挂着 */
export const DEFAULT_FETCH_TIMEOUT_MS = 15_000;

/**
 * 带超时 + 可中止的 fetch。
 *
 * 为什么单独包一层:`listRemoteModels` / `listOllamaModels` 打的是**用户填的**
 * baseUrl,端点只连不答(TCP 黑洞、网关不响应)时裸 `await fetch(...)` 永不 settle ——
 * HTTP 层的「刷新」会一直挂着。这里用 `Promise.race` 到点直接 reject(不指望底层
 * fetch 真会响应 abort),同时 abort 一个合成 controller 给它一个清理的机会。
 *
 * 底层 fetch 走 `opts.fetchImpl ?? fetch` —— 注入点在**这一层**留着,消费方
 * (`createAiCore({ fetchImpl })`)注的假 fetch 才不会被半路换成全局 fetch。
 */
export async function fetchWithTimeout(
  ...
  opts: {
    ...
    /** 不传 = DEFAULT_FETCH_TIMEOUT_MS */
    timeoutMs?: number;
    /** 调用方中止(用户点停止 / 路由断开)时一并中止 */
    abortSignal?: AbortSignal;
    /** 超时抛的错 —— 由调用方给一句用户能照着看的话 */
    timeoutMessage: string;
  },
): Promise<Response> {
  const fetchImpl = opts.fetchImpl ?? fetch;
  // 已经中止了就别再发请求 —— 底层 fetch 对「已 abort 的 signal」未必立刻抛(假实现就更不会)
  if (opts.abortSignal?.aborted) {
    const aborted = new Error('请求已中止');
  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        // handle 存下来、finally 里清 —— 正常路径(响应先回)不清的话这个 timer
...
export async function complete(opts: {
  ...
  try {
    ...
    // 先整体抹值**再**截断:反过来的话 key 可能正好被切一半,匹配不上就抹不掉了。
    // 这条日志绕不过 redact —— registerSecret 登记的凭据值在这里也要被抹掉,
    // 否则网关把请求头/正文里的 key 回显出来时会原样落进 stdout。
    const preview = redact(res.text.replace(/\s+/g, ' ').trim()).slice(0, 120);
