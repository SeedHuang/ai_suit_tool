...
describe('模型注册表', () => {
  ...
  // ★ A2：命中表内条目的分支返回的是**副本** —— 调用方就地改一下(按 UI 校正
  //   contextWindow)不该永久污染全局 MODELS 表,否则后续所有请求都拿到被改过的数字。
  it('getModelMeta 命中表内条目也返回副本 —— 就地改不污染全局表', () => {
    const before = getModelMeta('deepseek', 'deepseek-flash');
    const hitCopy = getModelMeta('deepseek', 'deepseek-flash');
    hitCopy.contextWindow = 1;
    hitCopy.note = '被就地改过';
    expect(MODELS['deepseek-flash']!.contextWindow).toBe(before.contextWindow);
    expect(getModelMeta('deepseek', 'deepseek-flash').contextWindow).toBe(before.contextWindow);
    expect(getModelMeta('deepseek', 'deepseek-flash').note).not.toBe('被就地改过');
  });

  it('getModelMeta 兜底分支同样返回新对象 —— 就地改不影响下一次', () => {
    const fallbackCopy = getModelMeta('custom', 'some-unknown-model-xyz');
    fallbackCopy.contextWindow = 1;
    expect(getModelMeta('custom', 'some-unknown-model-xyz').contextWindow).toBe(32_768);
  });

  // ★ C13：listModels 也返回副本 —— 与 getModelMeta 一致。返回表内引用的话,消费方
  //   (设置页下拉、/api/settings/models)就地改一下就会永久污染全局表。
  it('listModels 返回副本 —— 就地改不污染全局表', () => {
    const list = listModels('deepseek');
    list[0]!.contextWindow = 1;
    list[0]!.note = '被就地改过';
    expect(MODELS['deepseek-flash']!.contextWindow).toBe(1_024_000);
    expect(MODELS['deepseek-flash']!.note).toBe('DeepSeek-V4.1-Flash');
    expect(listModels('deepseek')[0]!.contextWindow).toBe(1_024_000);
    expect(listModels('deepseek')[0]!.note).toBe('DeepSeek-V4.1-Flash');
  });

  it('listModels() 全量列表同样是副本 —— 每条都不是表内引用', () => {
    const all = listModels();
    expect(all).toHaveLength(Object.keys(MODELS).length);
    expect(all.every((m) => m !== MODELS[m.model])).toBe(true);
    all[0]!.verified = false;
    expect(listModels()[0]!.verified).toBe(true);
  });
});
