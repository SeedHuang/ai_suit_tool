...
describe('redactDeep', () => {
  ...
  // ★ A5：seen 代表「当前祖先链」而不是「见过的所有对象」—— 同一对象出现在**兄弟**
  //   位置不是循环引用，不该被替换成 [循环引用] 把值丢掉（日志里同一个 config/ctx
  //   被引用两次很常见）。
  it('同一对象在数组里出现两次(非循环)不会被误判成循环引用', () => {
    const shared: Record<string, unknown> = { keep: 'me', api_key: 'sk-SECRET' };
    const out = redactDeep([shared, shared]) as { keep?: string; api_key?: string }[];
    expect(out).toHaveLength(2);
    for (const item of out) {
      expect(item.keep).toBe('me');
      expect(item.api_key).toBe('***'); // 该抹的还是抹
    }
  });

  it('{err, ctx:{cause:err}} 这种共享引用不丢值', () => {
    const err: Record<string, unknown> = { message: 'boom', token: 'sk-SECRET' };
    const out = redactDeep({ err, ctx: { cause: err } }) as {
      ctx: { cause: { message: string; token: string } };
    };
    expect(out.ctx.cause.message).toBe('boom');
    expect(out.ctx.cause.token).toBe('***');
  });

  it('真环仍然被拦下(出栈删除只放过共享引用,不放过环)', () => {
    const a: Record<string, unknown> = { name: '环' };
    a['self'] = a;
    const out = redactDeep(a) as { name: string; self: unknown };
    expect(out.name).toBe('环');
    expect(out.self).toBe('[循环引用]');
  });
});
