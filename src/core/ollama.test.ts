...
describe('listOllamaModels', () => {
  ...
  // ★ A4：/api/tags 只连不答时不能永挂 —— 到点抛一句能读的话。
  //   假 fetch 刻意**不理会 abort**：超时不能指望它配合。
  it('端点挂起 → 超时抛一句能读的话(不永挂)', async () => {
    const hung = (() => new Promise<Response>(() => {})) as unknown as typeof fetch;
    await expect(
      listOllamaModels('http://127.0.0.1:11434/v1', hung, 20),
    ).rejects.toThrow(/Ollama 响应超时\(20ms\)/);
  });

  it('abortSignal 透传给 fetch(调用方一 abort,请求就中断)', async () => {
    const controller = new AbortController();
    let seen: AbortSignal | null | undefined;
    // 真实 fetch 就是「signal 一 abort 立刻 reject」—— 这里照着模拟
    const f = ((_url: string | URL | Request, init?: RequestInit) => {
      seen = init?.signal;
      return new Promise<Response>((_, reject) => {
        seen!.addEventListener('abort', () => reject(new Error('请求被中止')));
      });
    }) as unknown as typeof fetch;

    const pending = listOllamaModels('http://127.0.0.1:11434/v1', f, undefined, controller.signal);
    await new Promise((r) => setTimeout(r, 0)); // 等 /api/tags 真的被调起来

    expect(seen).toBeInstanceOf(AbortSignal);
    expect(seen!.aborted).toBe(false);
    controller.abort();
    await expect(pending).rejects.toThrow(/请求被中止/);
    expect(seen!.aborted).toBe(true);
  });

  it('abortSignal 已经中止 → 立刻中止,一个请求都不发', async () => {
    const controller = new AbortController();
    controller.abort();
    let called = 0;
    const f = (async () => {
    ...
    await expect(
      listOllamaModels('http://127.0.0.1:11434/v1', f, undefined, controller.signal),
    ).rejects.toThrow(/中止/);
