 ...
 *
 * `seen` 代表的是**当前祖先链**,不是"见过的所有对象":进分支时 add、出来就 delete。
 * 只加不删会把「非循环、但共享同一引用」的结构(`[obj, obj]`、`{err, ctx:{cause:err}}`)
 * 在第二次出现时误判成 `[循环引用]`,静默丢值 —— 而那在日志里很常见。
 */
