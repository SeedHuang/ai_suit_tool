 ...
 *
 * 每个请求都带超时(默认 15s):端点只连不答时不让 `await` 永挂;`abortSignal`
 * 由调用方透传,用来取消(路由断开 / 用户点了别的)。
 */
export async function listOllamaModels(
  ...
  const req = (url: string, init: RequestInit) =>
    fetchWithTimeout(url, init, {
      ...
      timeoutMs,
      abortSignal,
      timeoutMessage: `Ollama 响应超时(${timeoutMs}ms)`,
    });

  const tagsRes = await req(`${root}/api/tags`, {});
  ...
  for (const name of names) {
    try {
      const showRes = await req(`${root}/api/show`, {
        method: 'POST',
