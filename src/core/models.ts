 ...
 *   (2026-09-18 补上),而表里那个 `deepseek-v4-flash` 既被记在方舟名下、又已退役
 *   (同日删掉)。
 * - **数字以注册表为准。** 那个接口只回 `{id, object, owned_by}`,**没有任何 token
 *   上限信息** —— 而这两个数字填错就等于 token 上限算错(`llm/ollama.ts` 开头那段
...
export async function listRemoteModels(opts: {
  ...
  /** 端点只连不答时的上限(ms);不传 15s —— 不给的话这个 await 会永久挂住路由 */
  timeoutMs?: number;
  /** 调用方中止时一并中止 */
  ...
  const res = await fetchWithTimeout(
    ...
    { headers: { authorization: `Bearer ${opts.apiKey || 'not-needed'}` } },
    {
      ...
      timeoutMessage: `拉模型列表超时(${timeoutMs}ms)`,
    },
  );
